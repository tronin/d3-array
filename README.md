...
If *radius* is specified, sets the radius of the transformation: on each iteration, the value of a point is transformed into the mean value of itself and the *radius* points of data surrounding it on the left and on the right (taking into account the edges). If *radius* is not specified, return the current radius. Defaults to 5.

<a name="blur_iterations" href="#blur_iterations">#</a> *blur*.<b>iterations</b>([*iterations*])

If *iterations* is specified, sets the number of iterations of the transformation. If not specified, return the current number. Defaults to 3.
